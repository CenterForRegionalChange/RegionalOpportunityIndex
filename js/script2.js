...
function createMap(j) { 'use strict';
    ...
    dojo.connect(tempMap, "onLoad", function () {        
        ...
        dojo.connect(maps[j], "onClick", function (evt) {
            var mapIndex, reportRowIndex, idQueryTask, idQuery, hideMap1 = '', hideMap2 = ''; 
            ...
            if ($('#rdoClickIdentify').get(0).checked) {
                ...
                reportRowIndex = $('#identify-report tr').length;
                
                if ($('#numPanelsSelected').val() < 2) { hideMap1 = ' style="display: none;"'; }
                if ($('#numPanelsSelected').val() < 3) { hideMap2 = ' style="display: none;"'; }
                
                $('#identify-report').append('<tr class="report-data"><td><span class="results-row-remove" id="rrr' + reportRowIndex + '" title="Remove this row">X</span></td>'
                    + '<td class="id-results-left"> </td><td class="id-results-right"> </td>'
                    + '<td class="id-results-left details-for-map1"' + hideMap1 + '> </td><td class="id-results-right details-for-map1"' + hideMap1 + '> </td>'
                    + '<td class="id-results-left details-for-map2"' + hideMap2 + '> </td><td class="id-results-right details-for-map2"' + hideMap2 + '> </td>'
                    + '</tr>');
                    
                idForMap(0, reportRowIndex, evt);
                idForMap(1, reportRowIndex, evt);
                idForMap(2, reportRowIndex, evt);
            }
        });
        ...
        dojo.connect(tempMap.graphics, "onClick", function() {
...
function idForMap(mapIndex, reportRowIndex, evt) { 'use strict';
    ...
    //$('#identify' + mapIndex).html("<tr><td>Loading...</tr></td>");
    idQueryTask.execute(idQuery, function (results) {
        var feature, renderField, idResults = results.features, fl = maps[mapIndex].getLayer("tempFeatLyr" + mapIndex), gfx, gfxId, $row, $cells;
    
        gfxId = reportRowIndex + '_' + mapIndex;
        $row = $('#identify-report tr').eq(reportRowIndex);
        $cells = $row.children();
        
        //maps[mapIndex].graphics.clear();        
        if (idResults.length > 0) {
            if (OP_MAPS[mapIndex].selectedGraphics.hasOwnProperty(gfxId)) {
                
            } else {
                feature = idResults[0];
                renderField = fl.renderer.attributeField;
                
                $cells.eq(mapIndex*2 + 1).html((feature.attributes.hasOwnProperty(fl.displayField) ? feature.attributes[fl.displayField] : "??"));
                $cells.eq(mapIndex*2 + 2).html((feature.attributes.hasOwnProperty(renderField) ? roundToDecimal(feature.attributes[renderField], 2) : "no data"));
            ...
            $cells.eq(mapIndex*2 + 1).html("");
            $cells.eq(mapIndex*2 + 2).html("");
        }
    });    
}
...
function idStartOver() { 'use strict';
    ...
    $('#identify-report tr.report-data').remove();
...
$(document).ready(function (e) { 'use strict';
    ...
    $('#identify-report').on('click', 'span.results-row-remove', function() {
        var i, reportRowIndex = $(this).get(0).id.replace('rrr', '');
        $(this).closest('tr').remove();
         
        for (i = 0; i < 3; i += 1) {
            if (OP_MAPS[i].selectedGraphics.hasOwnProperty(reportRowIndex + '_' + i)) {
                maps[i].graphics.remove(OP_MAPS[i].selectedGraphics[reportRowIndex + '_' + i]);
            }
        }
    });
